...
public class FsmStateEquipmentSelected : FsmStatePlayersEquipment
{
    ...
    IEnumerator DelayBeforeSetStateAtPlayer()
    {
    ...
    IEnumerator DelayBeforeSetStateInsideShop()
    {
        yield return null;
    ...
    public override void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0)) // Когда отпущена левая кнопка мыши
        {
            ...
            // в состояние FsmStateEquipmentAtUnit, если нет, то подразумевается, что оно было в магазине и мы возвращаем его туда FsmStateEquipmentInsideShop
            if (equipment.transformCurrentEquipmentPlace.parent == equipment.rectTransformTargetEquipmentPanelPlayer)
            {
                //fsm.SetState<FsmStateEquipmentAtPlayer>();
                CoroutineManager.Instance.StartManagedCoroutine(gameObject, DelayBeforeSetStateAtPlayer());
            }
