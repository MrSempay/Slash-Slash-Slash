...
public class FsmStateEnemy : FsmStateUnit
{
    ...
    private float baseUpOffsetForStartPointFindingPath = 0.5f; // дополгнительное "поднимание вверх" конечной и стартовой точки для поиска пути (чтоб был запас по отрицательному
    ...
    public void CalculateDrawPathChangeDirectionAndMove()
    {
        ...
        if (constraintTimeFlipXCoroutine == null)
        {
            ...
            // Обновляем путь, если необходимо (например, если игрок переместился)
    ...
    protected void Jump()
    {
        if (enemy.isGrounded) { // Если мы на земле
            if (path.corners.Length > 0) { // вот на ровном месте оно почему-то тут выдаёт 0 иногда 0_0
