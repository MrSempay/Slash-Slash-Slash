...
public class FsmStatePlayerL : FsmStateUnit
{
    ...
    public FsmStatePlayerL(Fsm fsm, GameObject gameObject) : base(fsm, gameObject)
    {
        fsmPlayer = fsm;
        player = (Player) unit;
        ...
        if (Input.touchCount > 0)
        {
                    ...
                    (touch.phase == TouchPhase.Ended || touch.phase == TouchPhase.Canceled))
            {
                if (player.CurrentStamina > 0)
                {
                    ...
                    if (Mathf.Abs(player.differenceXBetweenStartAndEndPositions) > 0.1)
                    {
                        HandleSwipe(player.endTouchPosition - player.startTouchPosition);
                        player.CurrentStamina--;
                    }
    ...
    #region //--- New move controller ---//

    private int activeFingerId = -1; // -1 — нет активного тача
    private bool pointerActive = false; // используется чтобы мышь не дублировалась при наличии тача
    private Vector3 startScreenPos;
    private const float MinSwipePixels = 20f; // порог в пикселях, настройте под устройство
    ...
    public void HandleTouches()
    {
        ...
        for (int i = 0; i < Input.touchCount; i++)
        {
            ...
            {
                // если UI — просто сбрасываем
                if (IsPointerOverUI(t.fingerId)) { ResetPointer(); continue; }

                Vector3 endScreenPos = new Vector3(t.position.x, t.position.y, 0f);

                if (player.CurrentStamina > 0 && Vector2.Distance(startScreenPos, endScreenPos) >= MinSwipePixels)
                {
                    player.endTouchPosition = player.mainCamera.ScreenToWorldPoint(Input.mousePosition);
                    player.endTouchPosition.z = 0;
                    player.differenceXBetweenStartAndEndPositions = player.endTouchPosition.x - player.startTouchPosition.x;
    ...
    {
        if (Input.GetMouseButtonDown(0) && !pointerActive)
        {
            // мышь: проверка UI (без fingerId)
        ...
        else if (Input.GetMouseButtonUp(0) && pointerActive)
        {
            if (player.CurrentStamina > 0)
            {
                ...
                if (Vector2.Distance(startScreenPos, endScreenPos) >= MinSwipePixels)
                {
                    player.endTouchPosition = player.mainCamera.ScreenToWorldPoint(Input.mousePosition);
                    player.endTouchPosition.z = 0;
                    player.differenceXBetweenStartAndEndPositions = player.endTouchPosition.x - player.startTouchPosition.x;
    ...
    Vector3 ScreenToWorldAtZ0(Vector3 screenPos)
    {
        // корректно конвертируем точку в мировые координаты на плоскости z = 0
        // для перспективной камеры: z должен быть расстоянием от камеры до плоскости z=0
        float z = -player.mainCamera.transform.position.z;
        screenPos.z = z;
        Vector3 world = player.mainCamera.ScreenToWorldPoint(screenPos);
        world.z = 0f;
        return world;
    }
    ...
    #endregion   //--- END new move controller ---//
