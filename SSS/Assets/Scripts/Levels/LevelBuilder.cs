...
public class LevelBuilder : MonoBehaviour
{
    ...
    protected IEnumerator SpawnEnemyByTimerNEW()
    {
        while (targetPointsForEnemyNEW.Any(targetPoint => targetPoint.Value.Any(e => e.amountEnemies > 0)))
        {
            ...
            // "завершения волны". Ибо если за эти 2 секунды будут убиты все враги, то при проверке spawnEnemyByTimerCoroutine на null в методе WasEnemiesWaveDestroyed мы не выдадим true
            {
                yield return new WaitForSeconds(timeBetweenEnemySpawnIteration);
            }
        }
    ...
    protected void SpawnEnemyNEW(Transform spawnPointTransform)
    {
        if (spawnPointTransform != null)
        {
            ...
            if (targetsForRandom.Count > 0)
            {
                ...
                foreach (TypesAndAmountEnemies infoEnemis in TargetPointsForEnemyNEW[randomTarget]) // проходимся по всем типам врагов и смотрим, нужно ли их ещё спавнить
                {
