...
public class ScenarioScript : MonoBehaviour
{
            ...
            reg = ctExtended.Register(() =>
            {
                ...
                if (sync != null)
                {
                    sync.Post(_ =>
                    {
                        ...
                        SkipTimerStuff lastSTF = Interlocked.Exchange(ref _skipTimerStuff, null); // КОРОЧЕ, ЭТО - рудиментные ДЕЙСТВИЯ. Ибо Dispose сам мы уже защитили.
    ...
    protected virtual void OnDestroy()
    {
        ...
        if (GameManager.Instance != null) // а вот так GameManager.Instance?.onDialogueStarted -= DialogueWasStarted; нельзя, увы
        {
            GameManager.Instance.onDialogueStarted -= DialogueWasStarted;
        }
        scriptPlayer.onUnitWasKilled -= UnitWasKilled;
