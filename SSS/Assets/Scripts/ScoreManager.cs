...
public class ScoreManager : MonoBehaviour
{
    ...
    public float styleMultiplier; // коэффициент усилени€ получени€ ресурсов от текущего ранга
    public float timeZeroizeKillComboTicks = 5; // врем€ дл€ сбрасывани€ текущего комбо за убийства. Ќачальное врем€ при загрузке сцены
...
#region ActionsCombo
    ...
    private int CurrentActionsCombo
    {
        ...
        set
        {
            ...
            if (value > 1) CurrentKillCombo += (int)(value * multiplayerActionCombo); // если комбо активностей равно 1 или 0, ничего основному комбо не добавл€ем
        }
    }
    ...
    public void UpActionCombo(int addictingCombo = 0, string nameAction = "")
    {
        if (!_blockActionCombo)
            ...
            {
                if (clasterSynergisticActions.Contains(nameAction)) // если интересующа€ нас активность есть в массиве
                {
                    if (CurrentActionsCombo == 0) // если мы только входим в комбо. ѕодразумеваетс€, что можем войти в комбо не с начала списка активностей, а с середины или под конец,
                                                  // а если комбо уже идЄт, то нужно провер€ть условие, €вл€етс€ ли текуща€ активность следующей по списку
                    {
                        ...
                        // ќстанавливаем предыдущую корутину (если она существует)
                        ...
                        if (clasterSynergisticActions.Count == 1 || clasterSynergisticActions.Count == _indexTargetActionInClaster)
                            CurrentActionsCombo = 0; // если у нас был всего один элемент в списке (вр€д ли) или прожата была последн€€ активность в списке
                    }
                    else
                    {
                        if (nameAction == clasterSynergisticActions[_indexTargetActionInClaster]) // провер€ем, что текуща€ активность соответствует требуемой следующей активности в комбо
                        {
                            ...
                            if (clasterSynergisticActions.Count == _indexTargetActionInClaster) // если текуща€ активность в комбо последн€€ в списке
                            {
    ...
    {
        yield return new WaitForSeconds(timeZeroizeActionComboTicks); // ∆дем 1 секунду

        // —брасываем комбо после задержки
        CurrentActionsCombo = 0;
        _zeroizeActionComboTicksCoroutine = null; // —брасываем ссылку на корутину
    }
    IEnumerator BlockActionComboForTime()
    {
        _blockActionCombo = true;

        yield return new WaitForSeconds(timeBlockActionCombo); // ∆дем 1 секунду
...
#region MasterOfSkills
    ...
    public struct RankProperties
    {
        ...
        public Action<bool> functionRank; // это ссылка на л€мбда-функцию, в которой будем получать ссылку на фунцию объекта ScoreManager.Instance. ’от€ можно было бы просто получать ссылки
                                        // на статические методы класса ScoreManager без л€мбда функции. 
    ...
    private void SetRank(int value)
    {
        foreach (var properties in rankProperties)
        {
            if (value >= properties.Value.min && value <= properties.Value.max)
            {
                ...
                return; // ¬ажно: выходим из цикла, как только нашли подход€щий диапазон
            }
        }

        Debug.LogError("«начение вне допустимого диапазона!"); // ≈сли не попали ни в один диапазон
    }
